...
CLI and local server startup settings come from a local TOML config file rather than environment variables alone. The default real config path is `candidate-search.toml` at the project root, and the committed template is `candidate-search.example.toml`. The config file is the primary place for OpenAI-compatible API key, base URL, preprocessing model name, embedding model name, and filesystem paths.
...
Missing config and missing index are separate failure domains. If the config file or required config values are missing, CLI commands fail directly and `serve-mcp` does not start. Required values are `openai.api_key`, `openai.base_url`, `models.preprocess`, `models.embedding`, `paths.raw_profiles`, and `paths.processed_dir`; empty strings count as missing. `candidate://index-status` is reserved for raw/preprocessed/index coverage and does not report configuration availability.

Search also requires `openai.api_key`, `openai.base_url`, and `models.embedding`, because query soft-preference texts are embedded at request time even when candidate-side embeddings already exist.
