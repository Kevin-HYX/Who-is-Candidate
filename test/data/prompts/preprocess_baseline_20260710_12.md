...
7. Never use `education`, `courses`, `certifications`, or licenses as a source for `skills_search_text`. Keep their names and status only in `education_search_text`. A related capability may still appear in skills only when `skills[]`, `summary`, `experience[].description`, or a specific occupational title independently supplies that capability; write only the independently supported capability and do not import credential or coursework wording. Study, exam preparation, and possession of a credential are not evidence that a skill was applied.
