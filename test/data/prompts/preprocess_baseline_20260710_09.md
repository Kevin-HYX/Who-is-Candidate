...
4. Group related evidence into coherent capability phrases instead of copying a raw keyword list. Prefer `SQL reporting, stored-procedure maintenance, and service-desk support` over `programming; deployment; software; sensors; reports; ticketing` when descriptions support the grouped wording. Do not copy raw `skills[]` as a keyword list.
5. Preserve discriminating names such as `Oomnitza`, `Microsoft Excel`, `Google Sheets`, `Salesforce Commerce Cloud`, `HubSpot`, or other exact source systems.
6. Courses may contribute only wording explicitly marked as `coursework`; study or exam preparation is not proficiency or a completed credential.
7. A specific occupational title may contribute a concise neutral skill concept, but a generic hierarchy title contributes nothing. Using a title-derived concept in responsibilities does not prevent one corresponding capability from appearing in skills; the two dimensions are independently permitted to use a specific occupational title.
8. Never convert `skills[]` into experience or responsibilities, and never claim that a listed skill was applied without work evidence.
9. If no valid content remains, return exactly `not_provided`.
